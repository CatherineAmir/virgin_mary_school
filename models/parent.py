...
class VmParent(models.Model):
    _name = "vm.parent"
    _description = "Parent"
    _inherit=["mail.thread", "mail.activity.mixin"]

    name = fields.Many2one('res.partner', 'Name', required=True,
                           domain="[('is_parent', '=', True)]",tracking=True)
    user_id = fields.Many2one('res.users', string='User', store=True)
    student_ids = fields.Many2many('vm.student', string='Student(s)', required=True,)
    mobile = fields.Char(string='Mobile',tracking=True)
    email = fields.Char(string='Email',tracking=True)
    active = fields.Boolean(default=True)
    relationship_id = fields.Many2one('vm.parent.relationship',
    ...
    def write(self, vals):
        for rec in self:
            res = super(VmParent, self).write(vals)
    ...
    def unlink(self):
        for record in self:
            ...
            return super(VmParent, self).unlink()
    # todo
